...
def run_batch_processing(
    ...
    if parallel == 1:
        ...
        with ThreadPoolExecutor(max_workers=parallel) as executor:
            futures = {executor.submit(trigger_dag_run, offset, batch_size, dry_run): offset for offset in offsets}
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch runner for sentiment pipeline (OpenAI version)",
    ...
    parser.add_argument("--total", type=int, required=True, help="Total number of records to process")
    parser.add_argument("--batch-size", type=int, default=1000, help="Records per DAG run (default: 1000)")
    parser.add_argument("--parallel", type=int, default=1, help="Number of parallel DAG runs (default: 1)")
    parser.add_argument("--start", type=int, default=0, help="Starting offset for resume (default: 0)")
    parser.add_argument("--delay", type=float, default=1.0, help="Delay between triggers in seconds (default: 1.0)")
    parser.add_argument("--dry-run", action="store_true", help="Print what would be triggered without executing")
